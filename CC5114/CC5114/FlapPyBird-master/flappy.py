...
FPS = 30 # Default Value: 30
SCREENWIDTH  = 288 # 288
SCREENHEIGHT = 512 # 512 - 400
...
def mainGame(movementInfo):
    ...
    global nextPipes, ITER, NN, userControl
    ...
    while True:
        ...
        # Actual position - Desired position draw line.
        """
        pygame.draw.aaline(SCREEN, (255, 0, 0), [playerxMidPos, playeryMidPos], [nextPipeWidthEnd, nextPipeGapCenter])
        pygame.draw.circle(SCREEN, (255, 0, 0), [int(nextPipeWidthEnd), int(nextPipeGapCenter)], 4)

        pygame.draw.circle(SCREEN, (0, 255, 0), [int(playerxMidPos), int(playeryMidPos)], 4)

        #pygame.draw.circle(SCREEN, (0, 255, 0), [int(nextPipes[0]['x']), int(nextPipes[1]['y'])], 4)  # Lower pipe
        pygame.draw.circle(SCREEN, (0, 255, 0), [int(nextPipes[0]['x']), int(nextPipes[1]['y'] - PIPEGAPSIZE)],4)  # Upper pipe

        pygame.draw.circle(SCREEN, (0, 255, 0), [int(nextPipeWidthEnd), int(nextPipes[1]['y'])], 4) #Lower pipe
        #pygame.draw.circle(SCREEN, (0, 255, 0), [int(nextPipeWidthEnd), int(nextPipes[1]['y'] - PIPEGAPSIZE)], 4) # Upper pipe
        ...
        # APRENDIZAJE SUPERVISADO CON MATRIZ DE ENTRADA
        #---------------------------------------------------------------------------------------------------------------

        # Screen grid representation.
        grid = screenGrid(34, 4, playerxMidPos, playeryMidPos, nextPipes[0]['x'], nextPipeWidthEnd, nextPipes[1]['y'] - PIPEGAPSIZE, nextPipes[1]['y'])
        ...
        for event in pygame.event.get():
            ...
            # User / Neural Network control.
            if event.type == KEYDOWN and event.key == K_RCTRL:
                userControl = not userControl
                # print("User Control: " + str(userControl))

            # User flapping.
            if event.type == KEYDOWN and (event.key == K_SPACE or event.key == K_UP) and userControl:
        ...
        if userControl:
            NN.train(gridToArray(grid), desiredOutput, 10)
            # print("Desired Output: " + str(desiredOutput))

        # Information.
        print("Neural Network Output: " + str(NN.getOutput()))

        # Neural Network flapping.
        if not userControl and NN.getOutput()[0] > 0.5:
            if playery > -2 * IMAGES['player'][0].get_height():
                playerVelY = playerFlapAcc
                playerFlapped = True
                SOUNDS['wing'].play()
        ...
        # Neural Network Training.
        if userControl:
            NN.train([horizontalDistance / 431, heightDifference / 397], [desiredOutput], 0.5)
            #print("Desired Output: " + str(desiredOutput))
        
        # Information.
        print("Neural Network Output: " + str(NN.getOutput()[0]))
        ...
        """
        # Neural Network Training.
        NN.train([horizontalDistance / 431, playeryMidPos / 397], [nextPipeGapCenter / 397], 0.5)

        # Information.
        print("Neural Network Output: " + str(NN.getOutput()[0]))
        print("Desired Output: " + str(nextPipeGapCenter / 397))

        # Flap condition.
        if playeryMidPos / 397 - NN.getOutput()[0]> 0.05:
            if playery > -2 * IMAGES['player'][0].get_height():
                playerVelY = playerFlapAcc
                playerFlapped = True
                SOUNDS['wing'].play()
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN and (event.key == K_SPACE or event.key == K_UP):
                if playery + playerHeight >= BASEY - 1:
                    return
            """
        # Automatic end for Neural Network Training.
        if playery + playerHeight >= BASEY - 1:
            return
...
def playerInGapGrid(playerVerticalArray, pipeVerticalArray):
    """Returns an arrau of a grid representation of the screen of the vertical player and pipes position."""
